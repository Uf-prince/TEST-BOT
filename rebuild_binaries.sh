...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -mod=vendor -ldflags="-s -w" -o gold-md-cgofree .
ls -lh gold-md-cgofree

echo "── [2/5] gold-md-lowram    (linux/amd64, CGO_ENABLED=0, -trimpath)"
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -mod=vendor -trimpath -o gold-md-lowram .
ls -lh gold-md-lowram

echo "── [3/5] gold-md-bot-static(linux/amd64, CGO_ENABLED=0, -s -w)"
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -mod=vendor -ldflags="-s -w" -o gold-md-bot-static .
ls -lh gold-md-bot-static

echo "── [4/5] gold-md-bot-arm64 (linux/arm64, CGO_ENABLED=0, -s -w)"
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -mod=vendor -ldflags="-s -w" -o gold-md-bot-arm64 .
ls -lh gold-md-bot-arm64

echo "── [5/5] gold-md-freebsd   (freebsd/amd64, CGO_ENABLED=0, -s -w)"
CGO_ENABLED=0 GOOS=freebsd GOARCH=amd64 go build -mod=vendor -ldflags="-s -w" -o gold-md-freebsd .
