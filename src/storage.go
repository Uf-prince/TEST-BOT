...
// ── STORADERA MIGRATION (2026-09-16, owner: jani) ──────────────────────────
// Storj ka free tier Feb-2024 me khatam + quota full tha ("You have reached
// your project storage limit") → fleet-wide writes dead → reconnect-war.
// Naya backend: Storadera (S3-compatible, 50GB / 30 din, 0 EUR, no card).
// Docs: https://storadera.com/integrations/universal-integration-manual
// Endpoint: https://eu-east-1.s3.storadera.com
const storjEndpoint = "eu-east-1.s3.storadera.com"
const storjRegion = "eu-east-1"
...
// storjDebug — OWNER REQUEST (2026-09-16): JSON logs LAZMI ("json debug b laga do"
...
func InitStorj() error {
	...
	for i := 1; i <= 10; i++ {
		...
		if !exists {
			if err := cli.MakeBucket(ctx, bucket, minio.MakeBucketOptions{Region: storjRegion}); err != nil {
...
// ── KV_OP JSON debug (owner directive 2026-09-16: "json logs lazmi") ──
...
func (u *Upstash) cmdCore(args ...string) (json.RawMessage, error) {
	...
	// LOCAL FLEET REGISTRY mirror (owner order): fleet claim/heartbeat/session
	// writes ko local file me bhi likho — taake reads hamesha local se hon.
	flMirrorKV(args)
	return res, err
}
...
// ── CROSS-NAMESPACE WHOLE-DB RESTORE (Path A fix) ──────────────────────────
// Problem: session-DB blobs are URL-ISOLATED — key = goldmd:sessiondb:<serverID>:blob
// where serverID = GOLDMD_SERVER_ID -> RENDER_EXTERNAL_URL -> RENDER_INSTANCE_ID
// -> hostname. Jab koi server NAYE hostname/instance pe boot hota hai (ya local
// sandbox pe jahan hostname random hota hai), uska apna namespace KHAALI hota
// hai — halanki doosre servers ke namespaces me valid session-DB blobs maujood
// hote hain. Pehle sirf apna namespace dekha jata tha => "nothing backed up yet"
// => session local folder me kabhi nahi aata. Ye fallback SAARE namespaces scan
// karta hai aur sabse NAYA (newest) blob uthata hai.
//
// WAR-SAFETY: ye sirf local goldmd.db populate karta hai. Connect-time guards
// (fleetHeldByLiveServer / fleetSessionOnlineElsewhere / WAR GUARD) ab bhi
// lagu rehte hain — yani agar session kisi doosre live server pe ONLINE hai to
// ye restore usse chheenta NAHI (wo guard StartSession pe rok deta hai).

type sessionDBNamespace struct {
	sid  string
	size int64
	mod  time.Time
}

// ListSessionDBNamespaces scans ALL shards for goldmd:sessiondb:<sid>:blob keys
// and returns the namespaces (newest first). Best-effort: shard errors skip.
func (u *Upstash) ListSessionDBNamespaces() []sessionDBNamespace {
	if !storj.Ready() {
		return nil
	}
	prefix := kvStringsPrefix + kvEncode(sessionDBKeyConst)
	seen := map[string]sessionDBNamespace{}
	ctx, cancel := context.WithTimeout(context.Background(), 25*time.Second)
	defer cancel()
	for _, sh := range storj.shards {
		if sh == nil || sh.client == nil {
			continue
		}
		objCh := sh.client.ListObjects(ctx, sh.bucket, minio.ListObjectsOptions{Prefix: prefix, Recursive: true})
		for oi := range objCh {
			if oi.Err != nil {
				continue
			}
			key := oi.Key
			// key = kv/strings/goldmd:sessiondb:<sid>:blob  (kvEncode colons ko
			// preserve karta hai, is liye seedha parse kar sakte hain).
			rest := strings.TrimPrefix(key, prefix)
			if !strings.HasSuffix(rest, sessionDBKeySuffix) {
				continue
			}
			sid := strings.TrimSuffix(rest, sessionDBKeySuffix)
			if sid == "" {
				continue
			}
			cur, ok := seen[sid]
			if !ok || oi.LastModified.After(cur.mod) {
				seen[sid] = sessionDBNamespace{sid: sid, size: oi.Size, mod: oi.LastModified}
			}
		}
	}
	out := make([]sessionDBNamespace, 0, len(seen))
	for _, ns := range seen {
		out = append(out, ns)
	}
...
// RestoreSessionDBAnyNamespace tries the local namespace first, then falls back
// to the newest blob from ANY other namespace. Returns (ok, sourceSID, err).
// sourceSID == "" means nothing was found anywhere.
func (u *Upstash) RestoreSessionDBAnyNamespace(path string) (bool, string, error) {
	// 1) apna namespace (normal path).
	if ok, err := u.RestoreSessionDB(path); err != nil {
		return false, "", err
	} else if ok {
		return true, u.serverID, nil
	}
	// 2) cross-namespace fallback — newest first, apna skip.
	namespaces := u.ListSessionDBNamespaces()
	for _, ns := range namespaces {
		if ns.sid == u.serverID {
			continue
		}
		key := sessionDBKeyConst + ns.sid + sessionDBKeySuffix
		r, err := u.cmd("GET", key)
		if err != nil {
			continue
		}
		...
		InfoLog("RESTORE: cross-namespace whole-DB restore OK — source=%s size=%d (local ns=%s khaali tha)", ns.sid, len(data), u.serverID)
		return true, ns.sid, nil
	}
	return false, "", nil
}
