...
func StartPanel(mgr *Manager, port int) {
	...
	mux.HandleFunc("/fleetdispatch", func(w http.ResponseWriter, r *http.Request) {
		// OWNER ORDER: cross-server dispatch BAND (default).
		if !fleetHandoffEnabled() {
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(map[string]any{
				"status": "disabled",
				"reason": "fleet handoff disabled (GOLDMD_FLEET_HANDOFF=off)",
			})
			return
		}
	...
	// ── /watchdogdispatch : Session Watchdog ka cross-server reconnect ──
	// POST {"jid":"923...@s.whatsapp.net","dispatch_id":"..."}
	// Session Watchdog (session_watchdog.go) offline session ka JID bhejta
	// hai; ye server KHUD verify karta hai (online-elsewhere + claim race +
	// WhatsApp truth 12s window) phir reconnect. Response:
	// {"status":"connected|online_elsewhere|logged_out|full|busy|error"}
	mux.HandleFunc("/watchdogdispatch", func(w http.ResponseWriter, r *http.Request) {
		handleFleetDispatch(mgr, w, r)
	})

	// ── /verifyhop : multi-hop verify chain (Request 11) ────────────────────
	// POST {"jid":"...","dispatch_id":"...","hop":0,"origin":"...","visited":[]}
	// Har hop pe server WhatsApp-truth verify karta hai (logged-in → stop;
	// logged-out → purge), phir agle server ko forward karta hai — SAME
	// verify wahan. Last hop pe session reconnect ho jata hai. Response:
	// {"status":"connected|online_elsewhere|logged_out|full|busy|error"}
	mux.HandleFunc("/verifyhop", func(w http.ResponseWriter, r *http.Request) {
		handleVerifyHop(mgr, w, r)
	})
	...
	// ── /diag : EXACT WAJA — session local folder me kyun nahi aaya ──
	// GET /diag?jid=923...@s.whatsapp.net
	// JSON: {fleet, device_local, in_fleet_set, blob_present, claim_holders,
	//        live_holders, held_by_live_server, online_elsewhere, reason}
	mux.HandleFunc("/diag", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		jid := strings.TrimSpace(r.URL.Query().Get("jid"))
		if jid == "" {
			w.WriteHeader(http.StatusBadRequest)
			_ = json.NewEncoder(w).Encode(map[string]any{"error": "jid required"})
			return
		}
		_ = json.NewEncoder(w).Encode(fleetAdoptReason(jid))
	})

	// ── /adopt : MANUAL FORCE-ADOPT (war-safe) ──
	// GET /adopt?jid=923...@s.whatsapp.net[&force=1]
	// force=1 ke bina sirf reason deta hai (dry-run). force=1 pe remote
	// holders ko /release bhej kar claim race jeet kar restore+connect.
	mux.HandleFunc("/adopt", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		// OWNER ORDER: cross-server force-adopt BAND (default).
		if !fleetHandoffEnabled() {
			_ = json.NewEncoder(w).Encode(map[string]any{
				"status": "disabled",
				"reason": "fleet handoff disabled (GOLDMD_FLEET_HANDOFF=off)",
		...
		if r.URL.Query().Get("force") != "1" {
			_ = json.NewEncoder(w).Encode(map[string]any{
				"status": "dry_run",
				"hint":   "force=1 lagao to zabardasti adopt karega (war-safe: remote /release pehle)",
				"diag":   fleetAdoptReason(jid),
			})
			return
		}
		ok, msg := fleetForceAdopt(jid)
		_ = json.NewEncoder(w).Encode(map[string]any{"status": msg, "ok": ok, "jid": jid, "server": fleetSelfID})
	})

	// ── /release : REMOTE RELEASE (force-adopt ka partner) ──
	// POST {"jid":"923...@s.whatsapp.net"} — is server pe chal rahi session
	// ka socket band karo (LOCAL cleanup only — Storj data SAFE). Doosra
	// server ise force-adopt se pehle call karta hai taake double-connect
	// war (WhatsApp stream-replace = logout) na ho.
	mux.HandleFunc("/release", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		// OWNER ORDER: cross-server release BAND (default).
		if !fleetHandoffEnabled() {
			_ = json.NewEncoder(w).Encode(map[string]any{
				"status": "disabled",
				"reason": "fleet handoff disabled (GOLDMD_FLEET_HANDOFF=off)",
		...
		var target *Session
		for _, s := range mgr.List() {
			if fleetUserPart(s.JID) == user {
				target = s
				break
			}
		}
		if target == nil {
			_ = json.NewEncoder(w).Encode(map[string]any{"status": "not_here", "jid": jid})
			return
		}
		mgr.surrenderSession(target, "remote /release request (force-adopt)")
		_ = json.NewEncoder(w).Encode(map[string]any{"status": "released", "jid": jid, "server": fleetSelfID})
	})
