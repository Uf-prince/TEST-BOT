...
func fleetBind(m *Manager, dbPath string) {
	...
	// LOCAL FLEET REGISTRY (owner order): session + claim + server info
	// hamesha local file se read ho. Boot pe disk se load; disk gayab ho to
	// Storj se EK BAAR (marker ke saath — dobara nahi). fleetBind AutoLoad
	// se PEHLE chalta hai, is liye AutoLoad ka zombie-guard bhi local
	// registry se padh sakta hai.
	flInit()
}
...
func fleetHeartbeat() {
	...
	// LOCAL WRITE-THROUGH: apni heartbeat local registry me bhi likho.
	flSetServer(fleetSelfID, fleetSelfURL(), time.Now().Unix(),
		fleetMgr.SlotsUsed(), maxPairedSessions())
...
func fleetOrphanSweep() {
	// OWNER ORDER: claim/handoff BAND — orphan claims release karna bhi
	// handoff ka hissa hai, isliye OFF.
...
func fleetHeartbeatMap() map[string]int64 {
	...
	// LOCAL-FIRST (owner order): server heartbeats local registry se.
	// Local me kuch bhi na ho to Storj fallback (ek baar) + local populate.
	if local := flHeartbeatMap(); len(local) > 0 {
		return local
	}
	r, err := fleetMgr.Redis.cmd("HGETALL", fleetServersHash)
	if err != nil {
...
// fleetClaimFresh: claim timestamp itni fresh hai ki bina probe trust karein?
...
func fleetServerURL(sid string) string {
	...
	// LOCAL-FIRST (owner order): server URL local registry se.
	if u := flServerURL(sid); u != "" {
		return u
	}
...
func fleetClaimHolders(jid string) map[string]int64 {
	...
	// LOCAL-FIRST (owner order): claim info local registry se. Local me
	// entry na ho to Storj fallback (ek baar) + local populate.
	if h, ok := flClaimHolders(jid); ok {
		return h
	}
...
// fleetHandoffEnabled: cross-server session CLAIM/HANDOFF engine ka master
// switch. OWNER ORDER (2026): fleet ka claim system (ek server doosre se
// session cheen kar apne paas chalana) BAND. Default OFF — sirf local 15s
// session-folder watchdog (watchReconnects) chalta hai. GOLDMD_FLEET_HANDOFF=on
// se purana behaviour wapas (debug/rollback ke liye).
func fleetHandoffEnabled() bool {
	// OWNER ORDER (2026-09-21): DEAD-SERVER FAILOVER AB DEFAULT ON.
	// PEHLE ye false tha → fleetOrphanSweep / fleetClaimAvailable /
	// fleetRestoreAndConnect / fleetOnPairSuccess sab early-return kar dete
	// the. Nateeja: jab ek Render server band (sleep / crash / redeploy)
	// hota tha to uske sessions KABHI dusre server pe online nahi hote the
	// — "sessions fleet system kaam nahi karta" ka ROOT CAUSE yahi tha.
	//
	// WAR-SAFETY ab bhi poori (fleetHolderAlive + fleetHolderRunsSession):
	//  • heartbeat FRESH (<3min)          → holder ALIVE → claim nahi chheena.
	//  • heartbeat stale + /health OK     → holder ALIVE (busy) → nahi chheena.
	//  • heartbeat stale + /health FAIL   → holder DEAD → claim release +
	//                                        failover marker + blob restore.
	//  • live holder ka ZOMBIE claim      → takeover (session wahan hai hi
	//                                        nahi).
	//  • fresh claim (<10min)             → trust, probe nahi (race-safe).
	// Band karna ho: GOLDMD_FLEET_HANDOFF=off.
	return envBool("GOLDMD_FLEET_HANDOFF", true)
}

func fleetClaimAvailable() {
	// OWNER ORDER: cross-server claim/handoff BAND (default). Sirf local
	// watchdog sessions ko revive karta hai — koi doosre server se cheena
	// nahi jata.
	if !fleetHandoffEnabled() {
		return
	}
	...
	for _, jid := range m.Redis.setMembers(fleetSessionsSet) {
		...
		if len(holders) > 0 {
			...
			for sid, ts := range holders {
				...
				// FRESH-CLAIM TRUST (WAR-LOOP FIX): claim <10min = holder ne
...
func fleetRestoreAndConnect(jid string) {
	// OWNER ORDER: cross-server claim+restore+connect BAND (default).
	if !fleetHandoffEnabled() {
		return
	}
	m := fleetMgr
...
// ════════════════════════════════════════════════════════════════════════
//   PATH C — FORCE-ADOPT (manual, war-safe)
// ════════════════════════════════════════════════════════════════════════
// Problem (Path C): agar session kisi DOOSRE live server pe ONLINE hai to
// fleetRestoreAndConnect ka online-elsewhere guard usse chheenta NAHI —
// session local folder me kabhi nahi aata. Owner ko ek MANUAL lever chahiye
// jo (a) exact waja bataye aur (b) chaahe to session ko zabardasti apne
// paas le aaye.
//
// WAR-SAFETY (khabardar rule): force-adopt pehle REMOTE holder ko /release
// bhejta hai (wo apna socket band karta hai, Storj data SAFE rehta hai),
// phir claim race jeet kar restore+connect karta hai. Isse double-connect
// war (WhatsApp stream-replace = logout) nahi hoti.

// fleetAdoptReason: exact waja — kyun session local folder me nahi aaya.
// /diag endpoint isi ko JSON me deta hai (owner ko "kaha se" pata chale).
func fleetAdoptReason(jid string) map[string]any {
	out := map[string]any{"jid": jid, "self": fleetSelfID}
	if fleetMgr == nil || fleetMgr.Redis == nil {
		out["fleet"] = "inactive"
		out["reason"] = "fleet inactive (Redis/Storj nahi) — single-server mode"
		return out
	}
	out["fleet"] = "active"
	out["device_local"] = fleetDeviceExists(jid)
	out["in_fleet_set"] = false
	for _, j := range fleetMgr.Redis.setMembers(fleetSessionsSet) {
		if j == jid {
			out["in_fleet_set"] = true
			break
		}
	}
	blob, ok := fleetMgr.Redis.getStringKV(fleetBlobPrefix + jid)
	out["blob_present"] = ok && blob != ""
	out["blob_bytes"] = len(blob)
	holders := fleetClaimHolders(jid)
	out["claim_holders"] = holders
	hb := fleetHeartbeatMap()
	live := []string{}
	for sid := range holders {
		if fleetHolderAlive(sid, hb) {
	...
	out["live_holders"] = live
	out["held_by_live_server"] = fleetHeldByLiveServer(jid)
	out["online_elsewhere"] = fleetSessionOnlineElsewhere(jid)
	// exact reason string
	switch {
	case !out["blob_present"].(bool) && !out["device_local"].(bool):
		out["reason"] = "NA blob NA local device — session data kahin nahi (purged/logged-out)"
	case out["device_local"].(bool):
		out["reason"] = "local device maujood — AutoLoad/StartSession se load ho jayega"
	case len(live) > 0:
		out["reason"] = "session kisi LIVE server pe hai (" + strings.Join(live, ",") + ") — online-elsewhere guard ne roka; /adopt?force=1 se le sakte ho"
	case out["online_elsewhere"].(bool):
		out["reason"] = "session remote /sessions me ONLINE mila — guard ne roka; /adopt?force=1 se le sakte ho"
	default:
		out["reason"] = "koi live holder nahi — fleet watchdog khud claim kar lega (ya /adopt?force=1 se foran)"
	}
	return out
}

// fleetReleaseRemote: ek remote server ko /release bhejta hai (uska socket
// band, Storj data SAFE). Force-adopt se pehle call hota hai — war-safe.
func fleetReleaseRemote(serverURL, jid string) bool {
	if serverURL == "" || jid == "" {
		return false
	}
	body, _ := json.Marshal(map[string]string{"jid": jid})
	url := strings.TrimRight(serverURL, "/") + "/release"
...
func fleetForceAdopt(jid string) (bool, string) {
	m := fleetMgr
	if m == nil || m.Redis == nil {
		return false, "fleet inactive"
	}
	if m.SlotsUsed() >= maxPairedSessions() {
		return false, fmt.Sprintf("server full (%d/%d)", m.SlotsUsed(), maxPairedSessions())
	}
	// 1. remote holders ko release bhejo (agar URL resolve ho).
	holders := fleetClaimHolders(jid)
	for sid := range holders {
		if sid == fleetSelfID {
			continue
		}
		if u := fleetServerURL(sid); u != "" {
			if fleetReleaseRemote(u, jid) {
				InfoLog("ADOPT: remote %s ko /release bheja (%s)", sid, jid)
			} else {
				WarnLog("ADOPT: remote %s /release fail (%s) — claim race pe bharosa", sid, jid)
			}
		}
	}
	// remote /sessions wave se bhi holders dhoondo (claim khaali ho sakta hai).
	for _, sid := range guardRemoteServers() {
		if sid == fleetSelfID {
			continue
		}
		if u := fleetServerURL(sid); u != "" {
			_ = fleetReleaseRemote(u, jid)
		}
	}
	// 2. claim race.
	now := strconv.FormatInt(time.Now().Unix(), 10)
	...
	if !won {
		_, _ = m.Redis.cmd("HDEL", fleetClaimPrefix+jid, fleetSelfID)
		return false, "claim race haar gaye — doosra server jeeta"
	}
	// 3. blob restore + connect.
	if !fleetDeviceExists(jid) {
		if err := fleetRestoreBlob(jid); err != nil {
			_, _ = m.Redis.cmd("HDEL", fleetClaimPrefix+jid, fleetSelfID)
			return false, "blob restore fail: " + err.Error()
		}
	}
	if err := m.StartSession(jid); err != nil {
		_, _ = m.Redis.cmd("HDEL", fleetClaimPrefix+jid, fleetSelfID)
		return false, "connect fail: " + err.Error()
	}
	OkLog("ADOPT: session %s force-adopt ho gaya (server %s)", jid, fleetSelfID)
	return true, "adopted"
}
...
// fleetClaimHeartbeat: connected sessions ke claims ko har 3 min me re-stamp
...
func fleetRefreshBlobs() {
	go func() {
		...
		for _, s := range m.List() {
			...
			fleetSaveBlob(s.JID)
			// OWNER ORDER: cross-server claim/handoff BAND — claim re-stamp
			// skip (sirf blob save chalta hai).
			if !fleetHandoffEnabled() {
...
func fleetOnPairSuccess(jid string, owner string) {
	...
	go func() {
		...
		// OWNER ORDER: cross-server claim/handoff BAND — claim stamp +
		// fleet registry set NAHI. Sirf owner setting save karo.
		if !fleetHandoffEnabled() {
			if owner != "" {
...
func fleetOnConnected(jid string) {
	...
	go func() {
		...
		// OWNER ORDER: cross-server claim/handoff BAND — sirf blob save
		// (restore ke liye) rakho, claim stamp NAHI.
		if !fleetHandoffEnabled() {
			fleetSaveBlob(jid)
			return
		}
...
// fleetScanAllFresh: .server ka FRESH scan — pehle health cache CLEAR, phir
// har server ka naya live /health probe (owner order: "har bar fresh servers
// unke pairs check kar ke msg aaye"). Isi wajah se .server ab purana/cached
// — yaani FAKE — data kabhi nahi dikhata (pehle fleetScanAll cached error
// entries 60s tak wapas deta tha, is liye server wapas online hone ke baad
// bhi OFFLINE dikhta rehta tha).
func fleetScanAllFresh() []fleetServerInfo {
	fleetHealthCacheMu.Lock()
	fleetHealthCache = map[string]fleetHealthCacheEntry{}
	fleetHealthCacheMu.Unlock()
	return fleetScanAll()
...
func fleetWriteHealth(w io.Writer, sessions int) {
	...
	// keepalive fragment add: /health pe hi proof milta hai ki anti-sleep
	// self-ping WAQAI chal raha hai (pings / every_min / last_ok).
	fmt.Fprintf(w, `{"bot":"GOLD-MD","status":"online","sessions":%d,"max":%d,"re":"%s","sid":"%s","used_mb":%.1f,"sess":%s,"keepalive":%s}`,
		sessions, maxPairedSessions(), fleetREStatus(), fleetSelfID, fleetEgressUsedMB(), sessJSON, keepaliveHealthJSON())
}
